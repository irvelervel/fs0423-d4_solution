...
for (let i = 0; i < starWarsCharacters.length; i++) {
  const objName = starWarsCharacters[i].name
  characters.push(objName)
  // characters.push(starWarsCharacters[i].name) // <-- in un unico passaggio
}
...
for (let i = 0; i < starWarsCharacters.length; i++) {
  switch (starWarsCharacters[i].eye_color) {
    ...
    default:
      console.log('404 - colore degli occhi non trovato!')
  }
}
...
// il while loop va avanti finchè non diventa falsa la sua condizione
while (i < starWarsCharacters.length) {
  // vogliamo usare questo while loop per navigare l'array starWarsCharacters
  // uso una i esterna come contatore

  // crewMass = crewMass + starWarsCharacters[i].mass
  crewMass += starWarsCharacters[i].mass // <-- v. abbreviata
  i++
}
...
const newStarWarsCharacters = Object.assign({}, starWarsCharacters) // crea un nuovo array COPIA IDENTICA di starWarsCharacters
...
console.log(newStarWarsCharacters)
...
for (let i = 0; i < femaleCharacters.length; i++) {
  for (let j = 0; j < characters.length; j++) {
    if (femaleCharacters[i].name === characters[j]) {
      ...
      console.log('elimino il personaggio')
